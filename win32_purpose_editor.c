...
typedef struct {
    ...
    HMODULE gamedll;
    editor_update_render *update_render;
}editor_code;
...
static editor_code 
win32_load_editor_code(char *game_dll_name, char *game_temp_dll_name)
{
    ...
    editor_code gamecode = {0};
    gamecode.update_render = editor_update_render_undefined;

    if(code)
    {
        gamecode.dllwritetime = GetLastFileTime(game_temp_dll_name);
        gamecode.gamedll = code;
        gamecode.update_render = (editor_update_render *)GetProcAddress(code, "editor_progress");
    }
    return gamecode;
}
static void
UnloadGamecode(editor_code *gamecode)
{
   gamecode->update_render = editor_update_render_undefined;
   if(gamecode->gamedll)
   {
                   ...
                   int nCmdShow)
{
    ...
    char *game_dll_name      = "purpose_editor.dll";
    char *game_temp_dll_name = "purpose_editor_temp.dll";
    editor_code gamecode = win32_load_editor_code(
    ...
    s_program_memory program_memory = {0};
...
//    render_GetTexture(&game_renderer, renderer ,gameState->game_asset_manager, 'FP01', 5);
    ...
    while(running)
	{
		...
		if(CompareFileTime(&gamecode.dllwritetime, &dllwritetime))
		{
			...
			gamecode = win32_load_editor_code(game_dll_name, game_temp_dll_name);
		}
