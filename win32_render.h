                 ...
                 render_api type)
{
	...
	game_renderer->texture_request_pixels = memory_area_push_size(
			area,
			total_texture_request_size);

	game_renderer->requested_textures = memory_area_push_array(
			area,
			texture_request,
			texture_request_max);
	for(u32 r = 0;
			r < game_renderer->requested_textures;
			r++)
	{
		texture_request *r = game_renderer->requested_textures + r;
	}
	game_renderer->first_open_texture_request = game_renderer->requested_textures[0];

	game_renderer->texture_request_max = texture_request_max;
#endif
    ...
    u8 *renderer_dll = "direct3d11.dll";
	switch(initial_values.api)
	{
